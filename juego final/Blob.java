import greenfoot.*;  // (World, Actor, GreenfootImage, Greenfoot and MouseInfo)

/**
 * Write a description of class bala here.
 * 
 * @author (your name) 
 * @version (a version number or a date)
 */
public class Blob extends Actor
{
    
    /**
     * Act - do whatever the bala wants to do. This method is called whenever
     * the 'Act' or 'Run' button gets pressed in the environment.
     */
    public void act()
    {
        
        move(20);
        eat();
        borrar();
        
    }
    
    public void eat()
    {
        Actor alien;
        Actor Blob;
        Blob = getOneObjectAtOffset(0, 0, Blob.class);
        alien = getOneObjectAtOffset(0, 0, alien.class);
        if (alien != null)
        {
            arena.arthurCount.add(-2);
            CrabWorld.arthurCount.add(-2);
            combat.arthurCount.add(-2);
            Greenfoot.playSound("dano.wav");
            getWorld().removeObject(Blob);
            
            if (getWorld() instanceof CrabWorld) {
                if (CrabWorld.arthurCount.getValue() <= 0)
                {
                getWorld().removeObject(alien);
                CrabWorld.musicfight.stop();
                Greenfoot.setWorld(new zombiwin());
                }
            }
            
            if (getWorld() instanceof arena) {
                if (arena.arthurCount.getValue() <= 0)
            {
                getWorld().removeObject(alien);
                CrabWorld.musicfight.stop();
                Greenfoot.setWorld(new zombiwin());
            }
            }
            
            if (getWorld() instanceof combat) {
                if (combat.arthurCount.getValue() <= 0)
            {
                getWorld().removeObject(alien);
                CrabWorld.musicfight.stop();
                Greenfoot.setWorld(new zombiwin());
            }
            }
        }
        
        Actor zombi;
        zombi = getOneObjectAtOffset(0, 0, zombi.class);
        if (zombi != null)
        {
            CrabWorld.jhonCount.add(-2);
            arena.jhonCount.add(-2);
            combat.jhonCount.add(-2);
            Greenfoot.playSound("dano.wav");
            getWorld().removeObject(Blob);
            
            if (getWorld() instanceof CrabWorld) {
                if (CrabWorld.jhonCount.getValue() <= 0){
                    getWorld().removeObject(zombi);
                    CrabWorld.musicfight.stop();
                    Greenfoot.setWorld(new alienwin());
                }
            }
            
            if (getWorld() instanceof arena) {
                if (arena.jhonCount.getValue() <= 0){
                    getWorld().removeObject(zombi);
                    CrabWorld.musicfight.stop();
                    Greenfoot.setWorld(new alienwin());
                }
            }
            
            if (getWorld() instanceof combat) {
                if (combat.jhonCount.getValue() <= 0){
                    getWorld().removeObject(zombi);
                    CrabWorld.musicfight.stop();
                    Greenfoot.setWorld(new alienwin());
                }
            }
        }
    }
    
    public void borrar()
    {
        Actor build = getOneIntersectingObject(carro.class);
        Actor car = getOneIntersectingObject(build.class);
        if (getX() <= 5 || getX() >= getWorld().getWidth() - 5)
        {
            getWorld().removeObject(this);
        }
        else if (getY() <= 5 || getY() >= getWorld().getWidth() - 5)
        {
            getWorld().removeObject(this);
        }
        else if (build != null)
        {
            getWorld().removeObject(this);
        }
        
        else if (car != null)
        {
            getWorld().removeObject(this);
        }
    }

}
